...
import { analyticsService } from '$lib/server/services/analytics.service.js';
...
export const actions = {
	...
	addBranch: async ({ request, locals }) => {
		...
		try {
			await footerService.addBranch({
				name,
				address: formData.get('branchAddress')?.toString() || ''
			});
	...
	updateCredentials: async ({ request, locals }) => {
		...
		if (!currentPassword) {
			return fail(400, { error: 'La contraseña actual es obligatoria.' });
		}

		if (!newEmail && !newPassword) {
			return fail(400, { error: 'No se enviaron datos para modificar.' });
		}
		...
		try {
			// Since Better Auth exposes `auth.api.changePassword` and `auth.api.changeEmail`
			// we will invoke them directly by passing the proper headers
			...
			if (newPassword) {
				...
				// Note: Better Auth typically returns an object if success or throws if invalid
			}

			if (newEmail && newEmail !== locals.user?.email) {
				// To change email, Better auth may require a verification workflow, but
				// as an admin tool we can use changeEmail and require current password if needed.
				// Assuming standard plugins configuration or database update if necessary.
				try {
					...
					console.error('Error in changeEmail API:', emailErr);
					// Fallback to update db directly since we are admin
					const { db } = await import('$lib/server/db');
					const { user } = await import('$lib/server/db/schemas/auth.schema.js');
			...
			return { credentialsUpdated: true };
		} catch (error) {
			console.error('Error updating credentials:', error);
			// Check if it's a known error shape, otherwise generic error
			const message = error instanceof Error ? error.message : 'Error al actualizar credenciales. Verifica tu contraseña actual.';
			return fail(400, {
				error: message
			});
		}
